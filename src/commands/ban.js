...
exports.run = (client, msg, args, data, errors) => {
    ...
    if (!modlogData.modlog) {
        userToBan.ban().then(banned => {
            msg.channel.sendMessage(`**${banned.user.username}** has been banned.`);
        ...
        userToBan.ban().then(banned => {
            msg.channel.sendMessage(`**${banned.user.username}** has been banned. I've logged it in the mod log.`);
        }).catch(err => { msg.channel.sendMessage(":no_entry_sign: **Error:**\n" + err); });
        let today = new Date();
