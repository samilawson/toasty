	...
	'add': (msg) => {
	  ...
	  if (error) {
		...
		if (url.includes("https://youtube.com/") || url.includes("https://www.youtube.com/") || url.includes("http://youtube.com/") || url.includes("http://www.youtube.com/") || url.includes("https://youtu.be/") || url.includes("https://www.youtu.be/") || url.includes("http://youtu.be/") || url.includes("http://www.youtu.be/")) {
			...
			var query = msg.content.replace(';add', '');
			addFromQuery(query);
		}
},
	'queue': (msg) => {
...
client.on('message', msg => {
  ...
  for (const URL of urls) {
    indico.contentFiltering(URL, {apiKey: config.indicoToken})
      .then((res) => {
        if (typeof res !== 'number' || res < (0.93 || 0.40)) return;
        if (!msg.guild.member(client.user).hasPermission("MANAGE_MESSAGES")) {
            return msg.reply("`Error:` Could not delete nsfw content because I do not have the `Manage Messages` permission!").catch(err => { msg.channel.sendMessage(`An error occured. Please report this:\n${err}`); });
        }
        msg.delete();
  ...
  if (msg.content.includes("https://discord.gg/") || msg.content.includes("http://discord.gg/") || msg.content.includes("discord.gg/") || msg.content.includes("https://discordapp.com/invite/") || msg.content.includes("http://discordapp.com/invite/")) {
      let dataNoinv = data[msg.guild.id] ? data[msg.guild.id] : {noinvite: "disabled"};
      if (dataNoinv.noinvite === "enabled") {
          if (!msg.guild.member(client.user).hasPermission("MANAGE_MESSAGES")) {
              return msg.reply("`Error:` Could not delete invite because I do not have the `Manage Messages` permission!").catch(err => { msg.channel.sendMessage(`An error occured. Please report this:\n${err}`); });
          }
